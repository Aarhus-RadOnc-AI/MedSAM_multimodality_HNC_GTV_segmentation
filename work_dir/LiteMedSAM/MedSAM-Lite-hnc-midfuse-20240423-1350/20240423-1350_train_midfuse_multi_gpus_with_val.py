...
class MedSAM_Lite_Midfuse(nn.Module):
    ...
    def forward(self, images, boxes):
        """
        # Fuse image modality dimension into batch dimension
        ...
        # Prepare a container for the embeddings of each modality
        modality_embeddings = []

        # Process each modality separately
        for i in range(N):
            # Select the i-th modality across all batches
            modality_images = images[..., i]  # shape (B, C, H, W)
            
            # Feed the selected modality images to the encoder
            modality_embedding = self.image_encoder(modality_images)  # Output shape (B, 256, 64, 64)
            
            # Store the embedding of this modality
            modality_embeddings.append(modality_embedding.unsqueeze(1))  # Add modality dimension
        
        # Stack all modality embeddings along the new modality dimension
        # modality_embeddings is a list of tensors of shape (B, 1, 256, 64, 64)
        stacked_embeddings = torch.cat(modality_embeddings, dim=1)  # shape (B, N, 256, 64, 64)

        # Fuse the embeddings along the modality dimension by averaging
        fused_image_embedding = stacked_embeddings.mean(dim=1)  # Output shape (B, 256, 64, 64)
