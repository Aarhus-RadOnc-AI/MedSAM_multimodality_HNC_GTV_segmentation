...
parser.add_argument("-img_path", type=str, default="/mnt/data/jintao/nnUNet/nnUNet_raw_data_base/nnUNet_raw_data/Task901_AUH/imagesTs/",
                    help="Path to the nii images, [default: data/FLARE22Train/images]")
parser.add_argument("-gt_path", type=str, default="/mnt/data/jintao/nnUNet/nnUNet_raw_data_base/nnUNet_raw_data/Task901_AUH/labelsTs",
...
npy_ts_path = join(output_path, prefix[:-1], "tests")
...
def preprocess(name, npy_path):
    ...
    if len(z_index) > 0:
        ...
        if save_nii:
            ...
            os.makedirs(join(output_path, prefix[:-1], 'sanity_tests'), exist_ok=True)
            sitk.WriteImage(
                gt_roi_sitk,
                join(output_path, prefix[:-1], 'sanity_tests', gt_name.split(gt_name_suffix)[0] + ".nii.gz"),
            )
...
if __name__ == "__main__":
    ...
    with mp.Pool(num_workers) as p: 
        with tqdm(total=len(test_names)) as pbar:
            ...
            for i, _ in tqdm(enumerate(p.imap_unordered(preprocess_ts, test_names))):
                pbar.update()
