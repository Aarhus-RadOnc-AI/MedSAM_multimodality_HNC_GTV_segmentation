...
parser.add_argument(
    ...
    type=str,
    required=True,
    help='directory of the original .nii.gz files',
)
parser.add_argument(
    '-device',
    type=str,
...
nii_save_dir = args.nii_save_dir
...
makedirs(nii_save_dir, exist_ok=True)

bbox_shift = 5
device = torch.device(args.device)
gt_path_files = sorted(glob(join(data_root, '*.npz'), recursive=True))
#nii_files = [file.replace('.npz', '_0000.nii.gz') for file in gt_path_files]
...
def MedSAM_infer_npz(gt_path_file):
    ...
    patient_id = npz_name.split('.npz')[0]
    nii_name = join(nii_img_dir, patient_id+'_0000.nii.gz')
    original_nii_img = sitk.ReadImage(nii_name)
    ...
    if (not isfile(join(pred_save_dir, task_folder, npz_name))) or overwrite:
        ...
        seg_3D[seg_3D>1] = 2 # label multiple GTV-N as 2 
        
        seg_sitk = sitk.GetImageFromArray(seg_3D)
        seg_sitk.SetSpacing(original_nii_img.GetSpacing())
        sitk.WriteImage(seg_sitk, join(nii_save_dir, patient_id+'.nii.gz'))
