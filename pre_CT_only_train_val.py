...
npz_ts_path = join(output_path, prefix[:-1], "vals")
...
def preprocess(name, npz_path):
    ...
    if len(z_index) > 0:
        ...
        if save_nii:
            ...
            sitk.WriteImage(
                gt_roi_sitk,
                join(output_path, prefix[:-1], 'sanity', gt_name.split(gt_name_suffix)[0] + "_gt.nii.gz"),
            )
...
if __name__ == "__main__":
    ...
    ts_names = [name for name in names if name in val_patient_ids]
